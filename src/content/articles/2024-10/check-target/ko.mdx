---
title: "조건 검사하는 데이터 구조 만들기 (TypeScript)"
summary: "Node.js + TypeScript 환경에서 조건 검사를 데이터화하는 방법을 다룹니다. 조건 데이터와 현재의 상황을 검사 함수에 집어넣으면 참/거짓이 나오도록 하는 과정입니다. 마지막에는 만든 함수가 잘 동작하는지 Vitest로 테스트까지 해봅니다."
...
import img0 from "./assets/i1.jpg";
...
<ArticleImage
  ...
  alt="타게팅을 하는 소년"
>
  Photo by [Norbert Braun](https://unsplash.com/@medion4you?utm_content=creditCopyText&utm_medium=referral&utm_source=unsplash) on [Unsplash](https://unsplash.com/photos/a-person-holding-a-baseball-bat-uU8n5LuzpTc?utm_content=creditCopyText&utm_medium=referral&utm_source=unsplash)
</ArticleImage>
...
<ArticleImage alt="조건 요구사항 도식화" img={img2}></ArticleImage>
...
`type` 필드는 Discriminated Unions 라는 겁나 어려운 단어로 조합된 유니온(`|`)을 사용할 때 흔히 쓰는 방식입니다. [타입스트립트에서 Narrowing이 필요한 이유](/ko/article/2023-07/why-need-narrowing/)라는 글에서 더 자세히 다루었으니 참조해주세요.
